...
from .unet_loader import NODE_CLASS_MAPPINGS as UNET_NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS as UNET_NODE_DISPLAY_NAME_MAPPINGS
from .lora_loader import NODE_CLASS_MAPPINGS as LORA_NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS as LORA_NODE_DISPLAY_NAME_MAPPINGS
from .vae_loader import NODE_CLASS_MAPPINGS as VAE_NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS as VAE_NODE_DISPLAY_NAME_MAPPINGS
from .clip_loader import NODE_CLASS_MAPPINGS as CLIP_NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS as CLIP_NODE_DISPLAY_NAME_MAPPINGS
from .image_loader import NODE_CLASS_MAPPINGS as IMAGE_NODE_CLASS_MAPPINGS, NODE_DISPLAY_NAME_MAPPINGS as IMAGE_NODE_DISPLAY_NAME_MAPPINGS

NODE_CLASS_MAPPINGS = {**UNET_NODE_CLASS_MAPPINGS, **LORA_NODE_CLASS_MAPPINGS, **VAE_NODE_CLASS_MAPPINGS, **CLIP_NODE_CLASS_MAPPINGS, **IMAGE_NODE_CLASS_MAPPINGS}
NODE_DISPLAY_NAME_MAPPINGS = {**UNET_NODE_DISPLAY_NAME_MAPPINGS, **LORA_NODE_DISPLAY_NAME_MAPPINGS, **VAE_NODE_DISPLAY_NAME_MAPPINGS, **CLIP_NODE_DISPLAY_NAME_MAPPINGS, **IMAGE_NODE_DISPLAY_NAME_MAPPINGS}
...
def has_models_or_subfolders(dir_path):
    ...
    try:
        for entry in os.listdir(dir_path):
            ...
            if os.path.isfile(entry_path) and entry.endswith(('.safetensors', '.pt', '.pth', '.bin', '.ckpt')):
                return True
    ...
    try:
        for entry in os.listdir(dir_path):
            ...
            if os.path.isfile(entry_path):
                ...
                if (entry_lower.endswith(image_extensions) or 
                    entry_lower.endswith(audio_extensions) or 
                    entry_lower.endswith(video_extensions)):
                    return True
            if os.path.isdir(entry_path):
    ...
    if os.path.exists(full_path):
        if os.path.isfile(full_path):
            ...
            if not full_path.endswith('.png'):
                name_without_ext = os.path.splitext(os.path.basename(full_path))[0]
                png_path = os.path.join(os.path.dirname(full_path), f'{name_without_ext}.png')
                if os.path.exists(png_path):
                    os.remove(png_path)
                pm_path = os.path.join(os.path.dirname(full_path), f'{name_without_ext}.pm')
                if os.path.exists(pm_path):
                    os.remove(pm_path)
        elif os.path.isdir(full_path):
            import shutil
            ...
            folder_preview_path = os.path.join(os.path.dirname(full_path), os.path.basename(full_path) + '.png')
    ...
    if os.path.exists(full_path):
        if os.path.isfile(full_path):
            ...
            if workflow_path.endswith('.json'):
                ...
                if os.path.exists(png_path):
                    os.remove(png_path)
                dot_png_path = os.path.join(os.path.dirname(full_path), '.' + os.path.basename(full_path)[:-5] + '.png')
...
async def rename_pm_workflow(request):
    try:
        ...
        if os.path.isfile(old_full_path) and old_full_path.endswith('.json'):
            ...
            new_name_without_ext = new_name[:-5] if new_name.endswith('.json') else new_name
